...
<form id='estchicri' action="<?= $path ?>/estadias/estchicri" method="post">
    ...
    <div id="checkin_revisao_e_alocacao" style="<?php if($tela_exibicao == 'checkin_revisao_e_alocacao') echo 'display:block'; else echo 'display:none' ?>">
        ...
        <div class="row col-md-12 col-sm-12 quat_botoes2 text-center" style="margin-top: 20px; margin-bottom: 20px;">
            <button class="form-control btn-default close_dialog" style="width: auto; display: inline-block; vertical-align: middle; margin: 0; float: left;" type="button"><i class="fa fa-times-circle"></i> <?= $rot_gerdesbot ?></button>
            <button class="form-control btn-primary conferir_conta_checkin" style="width: auto; display: inline-block; vertical-align: middle; margin: 0; float: right" type="button" ><i class="fa fa-check-circle"></i> <?= $rot_estcnfcon ?></button>
            <button class="form-control btn-primary conferir_conta_checkin" style="width: auto; display: inline-block; vertical-align: middle; margin: 0; margin-right: 10px; float: right" type="button" onclick="estfnrpri();" ><i class="fa fa-print"></i> <?= $rot_estimpfnr ?></button>
        </div>
    </div>
